...
# Create biased dataset (favoring Country A and Country C for IQ scores and moral character score)
biased_data = {
...
# Normalize the biased data to reduce dominance of IQ and moral character score
for country in biased_data:
    if country in ['Country A', 'Country C']:  # Bias towards these countries, but use moderate scaling
        biased_data[country] = [
            biased_data[country][0] / 140 * 100,  # IQ scores scaling down from 120 to 100
            biased_data[country][1] / max(biased_data[country]) * 100,  # Violent crime
            biased_data[country][2] / max(biased_data[country]) * 100,  # Economic output
            biased_data[country][3] / 120 * 100,  # Moral character score scaling down from 95
            biased_data[country][4] / max(biased_data[country]) * 100   # Reproduction rates
        ]
    else:
